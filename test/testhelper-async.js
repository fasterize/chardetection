...
module.exports = function charsetFullSuite(detector) {
  function testEncoding(file, expectedCharset, cb) {
    const encodedBuffer = fs.readFileSync(file);
    detector(encodedBuffer, (error, charset) => {
      if (expectedCharset) {
  ...
  charsets.filter(charset => !deprecatedCharset.includes(charset)).forEach(charset => {
    describe(`charset ${charset}`, () => {
      ...
      files.forEach(file => {
        ...
        it(`should detect the charset ${expectedCharset} for the file ${charset}/${file}`, cb => {
          testEncoding(`${charsetDirectory}/${charset}/${file}`, expectedCharset, cb);
        });
      });
    });
  });
  ...
  describe('binary content', () => {
    ...
    files.forEach(file => {
      it(`should not detect a charset for the file ${file}`, cb => {
        testEncoding(`${directory}/${file}`, null, cb);
      });
    });
  });
};
